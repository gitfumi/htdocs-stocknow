...
sub do_login {
    ...
    my $blog_id = $q->param('blog_id');
    my ( $blog, $auths );
    $blog  = MT::Blog->load($blog_id)        if ( defined $blog_id );
    $auths = $blog->commenter_authenticators if $blog;
...
sub loggedin_js {
    ...
    my $perms;
    $perms = $user->permissions($blog_id) if $blog_id && $user;
...
sub view_profile_method {
    ...
    my $blog
        = $blog_id ? $app->model('blog.community')->load($blog_id) : undef;
    return $app->errtrans("Invalid request") unless $blog;
...
sub save_profile_method {
    ...
    if ( $auth_type eq 'MT' ) {
        ...
        if ( scalar $q->param('pass') ) {
            ...
            if ( $pass =~ /[^\x20-\x7E]/ ) {
                $param{error} = $app->translate('Password contains invalid character.');
            }
