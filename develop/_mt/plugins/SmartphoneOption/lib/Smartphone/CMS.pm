...
sub on_pre_run {
    ...
    MT->request( 'OriginalEnableBlogStats',
        scalar( MT->config('EnableBlogStats') ) );
    MT->config( 'EnableBlogStats', 1 );

    # Enable legacy upload dialog
    MT->request( 'OriginalEnableUploadCompat',
        scalar( MT->config('EnableUploadCompat') ) );
...
sub on_take_down {
    my ( $cb, $app ) = @_;
    MT->config( 'EnableBlogStats', MT->request('OriginalEnableBlogStats') );
    MT->config( 'EnableUploadCompat',
        MT->request('OriginalEnableUploadCompat') );
}
